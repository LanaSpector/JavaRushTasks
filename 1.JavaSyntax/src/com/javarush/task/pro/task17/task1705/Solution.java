...
public class Solution {
    ...
    public static void runWorkingProcess() {
        for(Astronaut crew:astronauts) {
            if(crew instanceof Human) {pilot((Human)crew);}
            if(crew instanceof Human) {pilot((Human)crew);}
            if(crew instanceof Dog) {createDirection((Dog)crew);}
            if(crew instanceof Cat) {research((Cat)crew);}
        }
    }
